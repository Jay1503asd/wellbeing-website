...
import { AngerPage, AnxietyPage, DepressionPage, EDPage, GeneralPage, InternetPage } from "../pages/survey/surveyPages.tsx";
import { Question } from '../pages/survey/survey questions/anger.ts';
import ReportPage from './report.tsx';
import { general } from '../pages/survey/survey questions/general.ts';
...
export const SurveyParent: React.FC = () => {
  ...
  return (
    <Routes>
      <Route path = "/general" element = {<GeneralPage filledQuestions = {filledQuestions[0]} setFilledQuestions={(newQuestions) => {
      ...
      <Route path="/anger" element={<AngerPage filledQuestions={filledQuestions[1]} setFilledQuestions={(newQuestions) => {
        const updatedQuestions = [...filledQuestions];
        updatedQuestions[1] = newQuestions;
        setFilledQuestions(updatedQuestions);
      }} handleNavigate={(nextPage) => handleNavigate(nextPage, 0)} />} />

      <Route path="/anxiety" element={<AnxietyPage filledQuestions={filledQuestions[2]} setFilledQuestions={(newQuestions) => {
        const updatedQuestions = [...filledQuestions];
        updatedQuestions[2] = newQuestions;
        setFilledQuestions(updatedQuestions);
      }} handleNavigate={(nextPage) => handleNavigate(nextPage, 0)} />} />

      <Route path="/depression" element={<DepressionPage filledQuestions={filledQuestions[3]} setFilledQuestions={(newQuestions) => {
        const updatedQuestions = [...filledQuestions];
        updatedQuestions[3] = newQuestions;
        setFilledQuestions(updatedQuestions);
      }} handleNavigate={(nextPage) => handleNavigate(nextPage, 0)} />} />

    <Route path="/eating-disorder" element={<EDPage filledQuestions={filledQuestions[4]} setFilledQuestions={(newQuestions) => {
        const updatedQuestions = [...filledQuestions];
        updatedQuestions[4] = newQuestions;
        setFilledQuestions(updatedQuestions);
      }} handleNavigate={(nextPage) => handleNavigate(nextPage, 0)} />} />

    <Route path="/internet" element={<InternetPage filledQuestions={filledQuestions[5]} setFilledQuestions={(newQuestions) => {
        const updatedQuestions = [...filledQuestions];
        updatedQuestions[5] = newQuestions;
        setFilledQuestions(updatedQuestions);
      }} handleNavigate={(nextPage) => handleNavigate(nextPage, 0)} />} />
